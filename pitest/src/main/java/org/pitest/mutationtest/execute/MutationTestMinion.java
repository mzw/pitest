...
public class MutationTestMinion {
  ...
  public MutationTestMinion(MinionSettings plugins, final SafeDataInputStream dis,
      final Reporter reporter, final boolean enableAdamu) {
    this.dis = dis;
  ...
  public void run() {
    try {
      ...
      final MutationTestWorker worker = new MutationTestWorker(hotswap,
          engine.createMutator(byteSource), loader, paramsFromParent.fullMutationMatrix, this.enableAdamu);
  ...
  public static void main(final String[] args) {
    ...
    try {
      ...
      final MutationTestMinion instance = new MutationTestMinion(factory, dis, reporter, enableAdamu);
      instance.run();
    } catch (final Throwable ex) {
      ex.printStackTrace(System.out);
      LOG.log(Level.WARNING, "Error during mutation test", ex);
